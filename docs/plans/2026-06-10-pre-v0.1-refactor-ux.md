# Pre-v0.1.0 Refactor + UX Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `docs/superpowers/specs/2026-06-10-pre-v0.1-refactor-ux-design.md` — shared UI RPC client, worker handlers/entry split, auto-save toggle (off by default), popup removal (icon opens side panel, save moves into the panel), readable filter labels, and the Warm Editorial restyle.
...
Expected: `src/lib/**`, `src/worker/handlers.ts`, `src/worker/services/**`, `src/worker/settings/**` all ≥ 80% statements (the v0.1.0 gate).
...
Invoke the superpowers `verification-before-completion` skill before claiming done, then `requesting-code-review` before merging. After review, the M6 plan's remaining ship steps apply (demo GIF, version → `0.1.0.0`, tag `v0.1.0.0` — tag only with explicit human confirmation).

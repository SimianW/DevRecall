# Pre-v0.1.0 Refactor + UX Fixes — Design
...
Before shipping v0.1.0, remove the structural duplication that has accumulated across
...
- Structure refactor + top UX fixes, then ship v0.1.0.
- Auto-save toggle is **off by default** for new installs.
- Deferred to v0.1.1 (YAGNI): user-editable allowlist, saved-date on cards,
...
- Existing installs flip from silently-on to off — the correct v0.1.0 reset.
...
dark-mode QA), version → `0.1.0.0`, tag `v0.1.0.0`.

...
`processPage` now tags, then re-chunks `fullText` with `chunkTokens`, batch-embeds the chunk texts, and commits the embedded chunks + the `ready` flip in one transaction. On any failure (tag _or_ embed) the page is marked `failed` and its existing word chunks are left untouched, so it stays keyword-searchable (parent spec §5).
...
- **`MockLLMProvider` scope.** Hash-seeded vectors are stable and distinct but carry no semantics, so they prove the _pipeline_ (ready-with-vectors) but cannot prove ranking-by-meaning. M5c's retrieval tests therefore inject their own controlled-vector fake for the "matched by meaning" assertion; `MockLLMProvider` stays the general integration double.
